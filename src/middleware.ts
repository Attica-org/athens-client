...
export function middleware(request: NextRequest) {
  ...
  if (request.headers.get('referer') === null && request.nextUrl.pathname.split('/')[1] === 'agoras') {
    ...
    if (!token) {
      ...
      return NextResponse.redirect(`${process.env.NEXT_CLIENT_URL}/flow/enter-agora/${agoraId}`);
    }
  }
