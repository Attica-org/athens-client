    ...
    <article className={`flex justify-end items-start p-0.5rem ${isSameUser && 'pt-0'} pr-12 pb-0 h-full`}>
      <div className="flex justify-end items-end">
        ...
        <div className={`p-0.5rem ${isSameUser && 'pt-0'} flex flex-col justify-center items-end`}>
          ...
          <div className={`max-w-[60vw] whitespace-pre-line ${message.user.type === 'CONS' ? 'bg-red-200' : 'bg-blue-200'} rounded-tl-lg ${isSameUser && 'rounded-tr-lg'} rounded-bl-lg rounded-br-lg p-0.5rem  pl-10 pr-10 text-sm under-mobile:text-xs`}>
            {message.content}
          </div>
        </div>
      </div>
